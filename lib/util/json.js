...
exports.parse = function(text, classMappings) {
   ...
   */
  function classTransformReviver(k, v) {
    if (v && typeof v === 'object' && v.___class && classMappings[v.___class]) {
      v = Object.assign(new classMappings[v.___class](), v);
    }
  ...
  function objectRefsReviver(key, value) {
    if (value && typeof value === 'object') {
      ...
      //First stage
      while (chains.length) {
        let prevChain = chains[chains.length - 1];
        let prevChainHead = prevChain[0];

        if (value[prevChainHead.key] === prevChainHead.value) {// current object is a parent for {prevChain}
          let refId = prevChainHead.value[OBJECTREF];
