...
logger.initWinston = function(winstonLoggers) {
  if (winstonLoggers && require('cluster').isMaster) {
    logger.winston = require('./logger-winston')(winstonLoggers)
  }
}

logger.winstonLog = (type, ...args) => {
  if (logger.winston && logger.winston[type]) {
    logger.winston[type](...args)
  }
}
...
logger.info = logger.log = function() {
  if (!silent) {
    ...
    logger.winstonLog('info', ...arguments)
  }
}
...
logger.warn = function() {
  if (!silent) {
    ...
    logger.winstonLog('warn', ...arguments)
  }
}
...
logger.error = function() {
  if (!silent) {
    ...
    logger.winstonLog('error', ...arguments)
  }
}
...
logger.debug = function() {
  if (logger.verbose) {
    ...
    logger.winstonLog('debug', ...arguments)
  }
}
