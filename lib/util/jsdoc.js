...
const getJSDocRequire = () => {
  if (!_jsdocRequire) {
    ...
    //init jsdoc env
    const Config = _jsdocRequire('jsdoc/lib/jsdoc/config');
    const env = _jsdocRequire('jsdoc/lib/jsdoc/env');
    env.conf = new Config().get();
  }

  return _jsdocRequire;
};

const getJSDocParser = () => {
  const jsdocRequire = getJSDocRequire();

  const parser = jsdocRequire('jsdoc/lib/jsdoc/src/parser').createParser();
  const handlers = jsdocRequire('jsdoc/lib/jsdoc/src/handlers');
...
const parseJSDOC = fileName => {
  return getJSDocParser().parse([fileName]);
};
