...
function startRunner(runner, opts) {
  let runnerInstance;

  function handleTermination() {
    logger.info('Termination signal received. Shutting down.');

    runnerInstance.stop().then(exit, exitWithError);
  }
  ...
  return Promise.resolve()
    .then(() => {
      runnerInstance = runner.start(opts)
    })
    .catch(exitWithError);
}
...
function debug() {
  printVersion();
  buildOptions(true).then(opts => startRunner(CodeRunner.debug(), opts));
}
...
function pro() {
  ...
  buildOptions().then(opts => CodeRunner.pro().start(opts));
}
