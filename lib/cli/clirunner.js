...
program
  ...
  .option('--repo-path <path>', 'Repo path', DEFAULT_REPO_PATH)
  .action(cloud);
...
function cloud() {
  buildOptions()
    .then((opts) => {
      ...
      if (!opts.driverHost) throw 'error: missing required parameter [driver-host]';
      if (!opts.driverPort) throw 'error: missing required parameter [driver-port]';
