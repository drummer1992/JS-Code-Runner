...
function debug() {
  squeezeOptions(true).then(CodeRunner.debug);
}

function cloud() {
  squeezeOptions(true).then(CodeRunner.cloud);
}

function deploy() {
  squeezeOptions().then(CodeRunner.deploy);
}
...
function squeezeOptions(expandAppFiles) {
  ...
  options.backendless.apiServer = options.backendless.apiServer || program.apiServer;
