...
function ensureApp(options) {
  ...
  const gatherAppOptions = async(function* () {
    app.id = program.appId || app.id || (yield terminal.promptParam('Application ID'));
    app.apiKey = program.appKey || app.apiKey || app.secretKey || (yield terminal.promptParam('API Key'));
  });
