...
class Definitions {
  ...
  addFile(file, options) {
    if (!this.explicit && this.files.indexOf(file.relativePath) === -1) {
      ...
      classes.forEach(classDef => this.addType(classDef, options))
    }
  }

  addType(classDef, options) {
...
class ServerCodeModel {
  ...
  addService(service, file, options) {
    ...
    if (service.___def) {
      this.definitions.addType(service.___def, options)
    } else {
  ...
  addType(type, file, options) {
    ...
    if (type.___def) {
      this.definitions.addType(type.___def, options)
    } else {
      this.definitions.addFile(file, options)
    }
