...
class Definitions {
  ...
  addFile(file, options) {
    if (!this.explicit && this.files.indexOf(file.relativePath) === -1) {
      const classes = jsdoc.describeClasses(file.absolutePath)

      classes.forEach(classDef => this.addType(classDef, file, options))
    }
  }

  addType(classDef, file, options) {
    if (this.explicit) return

    if (!this.files.includes(file.relativePath)) {
...
class ServerCodeModel {
  ...
  addService(service, file, options) {
    ...
    if (service.___def) {
      this.definitions.addType(service.___def, file, options)
    } else {
  ...
  addType(type, file, options) {
    ...
    if (type.___def) {
      this.definitions.addType(type.___def, file, options)
    } else {
      this.definitions.addFile(file, options)
    }
