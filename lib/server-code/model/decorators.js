...
const resolvePropertyType = (type, target, key) => {
  const attributePath = `${target.constructor.name}.${key}`
  ...
  assert(type, `Type definition for ${attributePath} is not defined`)
  assert(type.prototype && type.prototype.constructor, `Type definition for ${attributePath} is not valid`)
