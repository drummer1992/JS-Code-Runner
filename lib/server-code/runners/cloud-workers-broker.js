...
function getKilledWorkerDetails(worker) {
  ...
  if (task.className) {
    const taskClassName = task.className.match(/\.\w+/)[0].substring(1)
    details.push(`service: ${taskClassName}`)
  }
  ...
  if (task.eventId) {
    ...
    if (event.provider.id === 'timer') {
      details.push(`handler: ${event.provider.id}(${task.target})`)
    } else {
      details.push(`handler: ${event.provider.id}(${event.name})`)
    }
  }
