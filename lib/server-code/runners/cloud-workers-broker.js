...
function getKilledWorkerDetails(worker) {
  ...
  if (task.deploymentModelName) {
    details.push(`deployed model: ${task.deploymentModelName}`)
  }

  if (task.className) {
    const taskClassName = task.className.match(/\.\w+/)[0].substring(1)
    details.push(`service name: ${taskClassName}`)
  }
  ...
  if (task.eventId) {
    ...
    details.push(`handler-event-name: ${event.provider.id}(${event.name})`)
  }
