      ...
      getAllWorkers().forEach(worker => {
        if (worker.heartbeat + this.heartbeatTimeout < Date.now()) {
          const task = worker.task,
              event = ServerCodeEvents.get(task.eventId),
              appId = worker.appId,
              method = task.method ? ', method: ' + task.method : '',
              deploymentModelName = task.deploymentModelName ? ', deployed service: ' + task.deploymentModelName : '',
              handlerEvent = task.eventId ? ', handler-event-name: ' + event.provider.id+'('+event.name+')' : ''
          this.killWorker(worker, (
            `Worker expired due to heartbeat timeout (${this.heartbeatTimeout}ms), ` +
            `the last ping was: ${worker.heartbeat && new Date(worker.heartbeat).toISOString()} ` +
            `[appId: ${appId}${method}${deploymentModelName}${handlerEvent}]`
          ))
        }
      })
