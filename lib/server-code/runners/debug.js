...
class DebugCodeRunner {
  constructor(opts) {
    ...
    //A separate redis connection for 'blpop' operation due to its blocking nature
    ...
    if (services.length) {
      return this.apiServer.unregisterDebugServices()
        .then(() => Promise.all(services.map((s) => {
          return this.apiServer.registerDebugService(s.name, this.model.describeService(s.name));
        })));
    }
  }
  ...
  processTask(task) {
    ...
    function sendResult(result) {
      ...
      if (result) {
        ...
        return msgBroker.rpush(task.id, result)
          .then(() => msgBroker.expire(task.id, 10))
