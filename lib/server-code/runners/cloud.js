...
exports.start = function(opts) {
  ...
  const driver = driverService(opts.driverHost, opts.driverPort, opts.driverRunnerId);
  ...
  function processTask(task) {
    ...
    return tasksExecutor.execute(task, opts)
      .then((result) => {
        ...
        return result && driver.sendResult(task.id, task.responseChannelId, task.result);
      });
  }
