...
module.exports = async function runMaster(opts) {
  ...
  const messageBroker = new MessagesBroker({
    connection        : opts.backendless.msgBroker,
    compressionEnabled: opts.compression.prod,
    gettersCount      : lowPriorityThreshold ? 2 : 1,
  })
  ...
  function onReadyForNextTasks() {
    ...
    logger.info('on ready for the next tasks', { currentWorkersLoad, availableWorkersCount })
    ...
    if (!taskRequests[tasksChannel]) {
      taskRequests[tasksChannel] = Promise.resolve()
        ...
        .then(async task => {
          ...
          const resetRequestBefore = workersBroker.getAvailableWorkersCount() > 2

          if (resetRequestBefore) {
            taskRequests[tasksChannel] = null
          }
