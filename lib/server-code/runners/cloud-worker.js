...
const DEFAULT_WORKERS_HEARTBEAT_INTERVAL = 10;

process.title = 'JS CodeRunner Worker';
...
function startHeartbeat(opts){
  const workersHeartbeatInterval = (opts.workersHeartbeat.interval || DEFAULT_WORKERS_HEARTBEAT_INTERVAL) * 1000;

  setInterval(() => process.send('heartbeat'), workersHeartbeatInterval);
}