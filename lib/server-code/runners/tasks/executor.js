 ...
 * @param {CodeRunnerTask} task
 */
const applySandbox = task => {
  const appUid = task.applicationId.replace('-', '');
...
executor.execute = function(task, opts, model) {
  return Promise.resolve()
    ...
    .then(() => opts.sandbox && applySandbox(task))
    .then(() => executeTask(task, model))
    .then(res => res !== undefined ? wrapResult(task, null, res) : res)
    .catch(err => wrapResult(task, err))
    .then(trace(task));
};
