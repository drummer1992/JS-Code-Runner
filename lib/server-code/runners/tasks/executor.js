...
function initClientSdk(task, opts) {
  if (task.initAppData) {
    const customDomain = task.invocationContextDto.customDomain
    const httpProtocol = task.invocationContextDto.httpProtocol || 'https:'
    ...
    if (customDomain) {
      Backendless.initApp(`${httpProtocol}//${customDomain}`)
    } else {
...
executor.execute = async function(task, opts, model) {
  ...
  try {
    ...
    console.startUserLoogin = true
    response = await executeTask(task, model)
    console.startUserLoogin = false
