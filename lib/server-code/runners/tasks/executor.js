...
function executeTask(task, rContext, model) {
  ...
  return Promise
    .race([
      taskExecutor(task, rContext, model),
      timeoutPromise
    ])
    .then(result => {
...
function wrapResult(task, rContext, error, result) {
  ...
  return JSON.stringify(wrapper.invocationResult(task.id, rContext, error, result))
}
...
executor.execute = async function(task, opts, model) {
  ...
  try {
    ...
    response = await executeTask(task, rContext, model)

    if (response !== undefined) {
      response = wrapResult(task, rContext, null, response)
    }

  } catch (error) {
    response = wrapResult(task, rContext, error)
  }
