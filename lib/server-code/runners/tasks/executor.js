 ...
 */
function wrapResult(error, result) {
  error && logger.error(logger.verbose && error.stack || (error.message || error));

  return JSON.stringify(wrapper.invocationResult(error, result));
}

const trace = request => response => {
  if (logger.verbose) {
    logger.debug('[TRACE.REQUEST]:', JSON.stringify(request));
...
executor.execute = function(task, opts, model) {
  return Promise.resolve()
    ...
    .then(res => res !== undefined ? wrapResult(null, res) : res)
    .catch(wrapResult)
    .then(trace(task));
};
