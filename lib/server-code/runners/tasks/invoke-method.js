...
function event(provider, name) {
  return events.types.byProvider[provider.id][name];
}
...
function mapArgsToExchange(task, model, req, res) {
  ...
  mapArgs(mappingRules[event.provider.id])(task, req, res);
  mapArgs(mappingRules[event.id])(task, req, res);
...
function mapExchangeToArgs(task, model, req, res) {
  ...
  mapArgs(mappingRules[event.provider.id], true)(task, req, res);
  mapArgs(mappingRules[event.id], true)(task, req, res);
