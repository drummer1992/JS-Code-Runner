...
module.exports = function(task, runnerOpts, model) {
  return new Promise((resolve, reject) => {
    ...
    function invoke() {
      ...
      if (!handler) {
        throw new Error(`${event.name}(${target}) event handler does not exist`);
      }
...
function buildModel(task, runnerOpts) {
  ...
  const modelDescriptor = require(codePath + '/model.json');
  ...
  const files = [handler.file];

  Object.keys(modelDescriptor.types).forEach(type => {
    files.push(modelDescriptor.types[type].file);
  });
