...
function execute(task, model) {
  return new Promise((resolve, reject) => {
    ...
    serviceInst.context = task.invocationContextDto;
    serviceInst.context.configurationItems.forEach(item => {
      serviceInst[item.name] = serviceInst[item.name] || item.value;
    });

    if (typeof serviceInst[task.method] !== 'function') {
      throw new Error(`${task.method} method does not exist or is not a function`);
    }

    const taskArgs = transformArguments(decodeTaskArguments(task.arguments, {}), model, serviceClassName, task.method);

    Promise.resolve(serviceInst[task.method].apply(service, taskArgs))
