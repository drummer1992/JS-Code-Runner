  ...
  function publishServices() {
    ...
    if (services.length) {
      ...
      if (services.length > 1) {
        throw new Error('Only one service is allowed for publish at the moment. ' +
          'Please, temporarily comment out extra "Backendless.ServerCode.addService" method calls');
      }
...
function addDependency(out, name) {
  ...
  try {
    ...
    // dependency not found at the root level. that's ok
  }
