...
exports.publish = function(opts) {
  ...
  function publishHandlers() {
    if (model.handlers.values.length) {
      apiServer.registerModel(model);
      apiServer.publish(zipped);
    }
  }
  ...
  Promise.resolve()
    .then(() => model = buildModel(process.cwd(), file.expand(opts.app.files, { nodir: true })))
    .then(() => zipped = generateZip(model, opts.app.files.concat(dependencyPatterns()), opts.keepZip))
