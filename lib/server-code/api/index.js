...
function addHandler(eventId, target, async, invoke, timer) {
  contextModel.addHandler({ eventId, target, async, invoke, timer: !!timer, file: currentContext() });
}
...
ServerCode.load = function(basePath, files, model) {
  ...
  files.forEach(file => {
    ...
    try {
      ...
      contextModel.addError(e, file, currentContext());
    }
  });
