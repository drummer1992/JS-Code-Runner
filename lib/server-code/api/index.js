...
ServerCode.load = function(basePath, files, model) {
  ...
  files.forEach(file => {
    ...
    try {
      ...
      const position = extractContextPositionFromError(e, file);

      contextModel.addError({ message: e.message, position }, file, currentContext());
      contexts.splice(0);
    }
  });
...
function extractContextPositionFromError(e, contextFile) {
  const matches = e.stack.match(new RegExp('at.*' + contextFile + ':(.*)\\)'));
...
function contextifyRequire(basePath) {
  return function(modulePath) {
    ...
    const absolutePath = Module._resolveFilename(modulePath, this);
    const relativePath = absolutePath.startsWith(basePath) ? absolutePath.substring(basePath.length + 1) : absolutePath;

    contexts.push({ absolutePath, relativePath });
