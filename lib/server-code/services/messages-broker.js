...
class MessagesBroker extends EventEmitter {
  constructor(opts, gettersCount, isMaster) {
    super()

    this.connectionInfo = opts.backendless.msgBroker
    this.compressionEnabled = (isMaster && opts.compression.prod) || (!isMaster && opts.compression.debug)
