...
const toServerModel = (applicationId, model) => ({
  applicationId,
  handlers: model.handlers.values().map(toServerHandler)
});
...
class ApiServerService {
  ...
  registerModel(model) {
    ...
    return request.post(this.appUrl + '/servercode/registermodel')
      .send(toServerModel(this.app.id, model))
  ...
  publish(model, appZipBuffer) {
    logger.info('Publishing Model to server');

    const formData = {
      'models-description': JSON.stringify(toServerModel(this.app.id, model)),
      code: {
        value  : appZipBuffer,
        options: {
          filename   : 'code',
