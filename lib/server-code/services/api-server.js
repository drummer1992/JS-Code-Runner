...
class ApiServerService {
  ...
  sendRequest(path, options) {
    options = Object.assign({
      url: `${this.serverUrl}/${this.app.id}/${this.app.secretKey}/servercode/${path}`
    }, options);
  ...
  registerRunner() {
    ...
    return this.sendRequest(`registerRunner/${LANG}`, { method: 'POST', json: true, body: { lang: LANG } })
      .then(function(res) {
        if (res.statusCode === 200) {
  ...
  registerDebugServices(services) {
    ...
    return this.sendRequest('services/debug', opts)
      .then(res => {
        if (res.statusCode === 200) {
          logger.info('Services successfully registered');
        } else {
          throw failureRespError(res, 'register services');
        }
      });
  }
  ...
  publishServices(appZipBuffer) {
    ...
    return this.sendRequest('services', opts)
      .then(res => {
        if (res.statusCode === 200) {
          logger.info('Services successfully published');
        } else {
          throw failureRespError(res, 'publish services');
        }
      });
  }
