...
class ApiServerService {
  ...
  registerModel(model) {
    ...
    return request.post(this.appUrl + '/servercode/registermodel')
      .send(Object.assign(model.toServerModel(), {
        applicationId      : app.id,
        deploymentModelName: app.model || 'default'
      }))
  ...
  publish(model, modelZip) {
    ...
    const modelName = this.app.model || 'default';

    const formData = {
      modelName,
      'models-description': JSON.stringify({ //TODO: remove me after server remove
        deploymentModelName: modelName
      }),
      code                : {
        value  : modelZip,
        options: {
