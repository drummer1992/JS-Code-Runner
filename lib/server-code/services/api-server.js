...
class ApiServerService {
  ...
  registerDebugService(serviceName, serviceXml) {
    logger.info(`Registering Service ${serviceName} for debug`);
    ...
    return this.sendRequest('services/debug', opts)
      .then(res => {
        if (res.statusCode === 200) {
          logger.info(`Service ${serviceName} successfully deployed`);
        } else {
          throw new Error(`Unable to deploy service ${serviceName}. ${parseError(res)}`);
        }
      });
  }

  publishService(serviceName, appZipBuffer) {
